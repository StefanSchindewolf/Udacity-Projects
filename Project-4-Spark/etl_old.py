...
def create_spark_session():
    ...
    sc_jsc.hadoopConfiguration().set("fs.s3n.awsAccessKeyId", config.get('AWS', 'AWS_ACCESS_KEY_ID'))
    sc_jsc.hadoopConfiguration().set("fs.s3n.awsSecretAccessKey", config.get('AWS', 'AWS_SECRET_ACCESS_KEY'))
    sc_jsc.hadoopConfiguration().set("fs.s3n.endpoint", "s3.amazonaws.com")
    return spark
...
def process_song_data(spark, input_data, output_data):
    ...
    try:
        songs.write.parquet((output_data + 'songs/'), mode='overwrite', compression='gzip', partitionBy=['year', 'artist_id'])
        print('Parquet files for table songs written')
    except Exception as e:
        print('Writing output files failed: ', e)
    ...
    try:
        ...
        print('Parquet files for table artists written')
    except Exception as e:
        print('Writing output files failed: ', e)
...
def process_log_data(spark, input_data, output_data):
    ...
    # read log data file
    staging_events = spark.read.json((input_data + 'log-data/*/*/2018*.json'))
    ...
    try:
        ...
        print('Parquet file for table users written')
    except Exception as e:
        print('Writing output files failed: ', e)
    ...
    try:
        ...
        print('Parquet files for table songplays written')
    except Exception as e:
        print('Writing output files failed: ', e)
    ...
    try:
        ...
        print('Parquet files for table time written')
    except Exception as e:
        print('Writing output files failed: ', e)
...
def main():
    ...
    for bucket in list_of_buckets['Buckets']:
        if bucket_name in bucket.get('Name'): print('Bucket already exists')
        else:
